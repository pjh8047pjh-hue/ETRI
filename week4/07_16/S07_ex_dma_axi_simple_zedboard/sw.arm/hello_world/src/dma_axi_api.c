...
//--------------------------------------------------------------------
static uint32_t ADDR_DMA_START=0x50000000;
static uint32_t CSRA_CONTROL=(0x50000000+0x30);
static uint32_t CSRA_NUM    =(0x50000000+0x40);
static uint32_t CSRA_SRC    =(0x50000000+0x44);
static uint32_t CSRA_DST    =(0x50000000+0x48);

//--------------------------------------------------------------------
// bit position
#define DMA_ctl_en     31
#define DMA_ctl_ip     1
#define DMA_ctl_ie     0

#define DMA_num_go     31
#define DMA_num_busy   30
#define DMA_num_done   29
#define DMA_num_chunk  16
#define DMA_num_bnum   0

//--------------------------------------------------------------------
// bit mask
#define DMA_ctl_en_MSK     0x80000000
#define DMA_ctl_ip_MSK     0x2
#define DMA_ctl_ie_MSK     0x1

#define DMA_num_go_MSK     0x80000000
#define DMA_num_busy_MSK   0x40000000
#define DMA_num_done_MSK   0x20000000
#define DMA_num_chunk_MSK  0x00FF0000
#define DMA_num_bnum_MSK   0x0000FFFF
...
int dma_axi_enable( int en, int ie )
{
    volatile uint32_t value;
    value = 0;
    if (en) value |= DMA_ctl_en_MSK;
    if (ie) value |= DMA_ctl_ie_MSK;
               ...
               , int time_out)
{
    ...
    REGWR(CSRA_DST, dst);
    REGWR(CSRA_SRC, src);
    value  = DMA_num_go_MSK | (chunk&0xFF)<<16 | (bnum&0xFFFF);
