...
후보 B의 고정 routing 구조를 기준으로 다음 두 파일을 추가했다.

| 파일 | 역할 |
|---|---|
| [rtl/depthwise_3x3_core.sv](rtl/depthwise_3x3_core.sv) | row/col/tap counter, padding mask, IFM/OFM 주소 및 채널 단위 제어 |
| [rtl/depthwise_mac_dsp48.sv](rtl/depthwise_mac_dsp48.sv) | signed 9-tap 누산 wrapper; 초기에는 DSP inference, 최종에는 DSP48 Macro IP로 교체 가능 |

현재 RTL 초안의 전제:

- IFM BRAM read latency는 1 cycle
- 입력과 weight는 signed 16 bit 기본값
- Accumulator는 signed 48 bit 기본값
- 출력마다 padding을 포함해 정확히 9개의 token을 MAC에 전달
- Padding token은 IFM BRAM을 읽지 않고 MAC operand를 0으로 고정
- 한 번의 `start` 명령으로 한 채널의 196개 출력을 처리
- 채널 weight 9개는 `start` 전에 load
- BN 및 ReLU6는 이 코어 밖에서 후처리

`depthwise_mac_dsp48.sv`의 포트는 유지하고 내부만 generated DSP48 Macro instance로
교체하면 상위 depthwise controller를 수정하지 않고 IP 버전을 비교할 수 있다.
