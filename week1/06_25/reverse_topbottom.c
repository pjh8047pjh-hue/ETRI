...
int main() {
	...
	for(int i=0; i<height; i++){
		for(int j=0; j<width; j++){
			int bottomindex = i * rowSize + j * bytesPerPixel;
			int topindex = (height - 1 - i) * rowSize + j * bytesPerPixel;
			
			// topindex와 bottomindex의 BGR 값을 서로 바꿔서 pOdata에 저장
			pOdata[topindex + 0] = pIdata[bottomindex + 0];
			pOdata[topindex + 1] = pIdata[bottomindex + 1];
			pOdata[topindex + 2] = pIdata[bottomindex + 2];
		}
	}
	
	FILE *fpOut = fopen("reversed_lenna.bmp", "wb");
