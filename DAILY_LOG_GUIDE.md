...
Notion은 자세한 내용이 코드나 commit에 이미 있으면 복사하지 말고 링크만 단다. 다만 "무엇을 했는지" 나열이 아니라 "무엇을 새로 이해했는지"를 설명하는 글이어야 한다 — 아래 "Notion 형식" 참고.
...
## 나의 질문
**Q.** 코드를 보다 스스로 던진 질문.
**A.** 코드를 근거로 한 답. (모르면 "확인 필요"라고 쓰고 다음 할 일에 남긴다.)
